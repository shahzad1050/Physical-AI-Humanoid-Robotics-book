...
async def startup_event():
    ...
    # For testing purposes, we'll allow missing API keys but warn the user
    required_env_vars = ['COHERE_API_KEY', 'NEON_CONNECTION_STRING']
    ...
    # Log if optional API keys are missing (for info only)
    if not os.getenv('OPENAI_API_KEY'):
        logger.warning("OPENAI_API_KEY not set - ChatBot will run in offline mode")

    # Initialize RAG Engine
    rag_engine = RAGEngine(table_name="docusaurus_content")

    # Initialize ChatBot
    # For the ChatBot, we'll handle missing API key gracefully in the ChatBot class itself
    chatbot = ChatBot(
